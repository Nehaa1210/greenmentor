...
const Task = () => {
  ...
  useEffect(() => {
    if (mode === "update") {
      const config = { url: `https://greenbackend-1.onrender.com/api/tasks/${taskId}`, method: "get", headers: { Authorization: authState.token } };
  ...
  const handleSubmit = e => {
    ...
    if (mode === "add") {
      const config = { url: "https://greenbackend-1.onrender.com/api/tasks", method: "post", data: formData, headers: { Authorization: authState.token } };
      fetchData(config).then(() => {
        navigate("/");
      });
    }
    else {
      const config = { url: `https://greenbackend-1.onrender.com/api/tasks/${taskId}`, method: "put", data: formData, headers: { Authorization: authState.token } };
      fetchData(config).then(() => {
        navigate("/");
      });
    }
