...
export const postLoginData = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post('https://greenbackend-1.onrender.com/api/auth/login', { email, password });
    dispatch({
      type: LOGIN_SUCCESS,
      payload: data,
    });
...
export const saveProfile = (token) => async (dispatch) => {
  try {
    const { data } = await axios.get('https://greenbackend-1.onrender.com/api/profile/', {
